...
export async function sendWelcomeLink({
  ...
  await sendMail({
    to,
    subject: "Willkommen bei H+M Operation Cloud",
    text: `Hallo ${name},\n\nDein Account wurde eingerichtet. Klicke auf den folgenden Link um dich einzuloggen:\n${link}\n\nDer Link ist 24 Stunden gültig.\n\nH+M Operation Cloud`,
    html: `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="font-family: system-ui, sans-serif; max-width: 500px; margin: 40px auto; color: #111;">
  <h2 style="margin-bottom: 8px;">Willkommen, ${name}!</h2>
  <p style="color: #555; margin-bottom: 8px;">Dein Account bei <strong>H+M Operation Cloud</strong> wurde eingerichtet.</p>
  <p style="color: #555; margin-bottom: 32px;">Klicke auf den Button um dich zum ersten Mal einzuloggen:</p>
  <a href="${link}" style="
    display: inline-block;
    background: #111;
    color: white;
  ...
  ">Jetzt einloggen</a>
  <p style="margin-top: 32px; font-size: 13px; color: #888;">
    Der Link ist 24 Stunden gültig. Danach kannst du jederzeit einen neuen Link über ${appUrl} anfordern.
  </p>
  <hr style="margin: 32px 0; border: none; border-top: 1px solid #eee;">
  <p style="font-size: 12px; color: #aaa;">H+M Operation Cloud · ${appUrl}</p>
</body>
